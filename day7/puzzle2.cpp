...
#define MIN_DISK_SPACE_REQUIRED 30000000
...
int main(){
    ...
    int ans = 0;
    int unused_space = DISK_SPACE - jjjjjj_filesystem["/"].total_size;
    int need_to_free = MIN_DISK_SPACE_REQUIRED - unused_space;
    set<int> ordered_sizes;
    for(auto e = jjjjjj_filesystem.begin(); e != jjjjjj_filesystem.end(); e++){
        Directory d = e->second;
        ordered_sizes.insert(d.total_size);
    }
    for(int dir_size : ordered_sizes){
