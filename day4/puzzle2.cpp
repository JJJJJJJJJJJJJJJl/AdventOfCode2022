...
bool overlapping(string pair_assignment){
    vector<int> pair_assignment_ranges = process_pair_assignment(pair_assignment);
    map<int,int> section_occs;
    for(int i=pair_assignment_ranges[0]; i<=pair_assignment_ranges[1]; i++){
        section_occs[i]++;
    }
    for(int i=pair_assignment_ranges[2]; i<=pair_assignment_ranges[3]; i++){
        section_occs[i]++;
        if(section_occs[i] > 1) return true;
    }
    return false;
}
...
int main(){
    ...
    while(getline(cin, pair_assignment)){
        overlapping(pair_assignment) ? ans++ : NULL;
    }
    cout << ans << endl;
    return 0;
}